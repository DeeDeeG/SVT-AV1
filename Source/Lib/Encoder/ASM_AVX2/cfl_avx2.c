...
/*!< Returns a vector where all the (32-bits) elements are the sum of all the lanes in a. */
...
/*staticINLINE*/ void eb_subtract_average_avx2(int16_t *pred_buf_q3, int32_t width, int32_t height,
    ...
    if ((width == 4) || (width == 8)) {
        ...
        /*!< To maximize usage of the AVX2 registers, we sum two rows per loop iteration */
        const int32_t step = 2 * CFL_BUF_LINE_I256;

        __m256i sum = _mm256_setzero_si256();
        /*!< For width 32, we use a second sum accumulator to reduce accumulator dependencies in the loop. */
        __m256i sum2;
