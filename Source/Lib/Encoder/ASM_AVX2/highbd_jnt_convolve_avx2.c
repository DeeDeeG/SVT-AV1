    ...
    if (!(w % 16)) {
        ...
        for (i = 0; i < h; i += 2) {
            for (j = 0; j < w; j += 8) {
                ...
                const __m256i src_10 =
                    _mm256_insertf128_si256(_mm256_castsi128_si256(src_row_0), src_row_1, 1);
                ...
                if (w - j < 8) {
                    if (do_average) {
                        ...
                        _mm_storel_epi64((__m128i *)(&dst0[i * dst_stride0 + j + dst_stride0]),
                                         res_1);
                    } else {
                        const __m256i res_unsigned_16b = _mm256_adds_epu16(res, offset_const_16b);
    ...
    for (j = 0; j < w; j += 8) {
        ...
        /*!< Vertical filter */
        {
            ...
            for (i = 0; i < h; i += 2) {
                ...
                res_a_round =
                    _mm256_sra_epi32(_mm256_add_epi32(res_a_round, round_const_y), round_shift_y);
                ...
                if (w - j < 8) {
                    ...
                    res_b_round = _mm256_sra_epi32(_mm256_add_epi32(res_b_round, round_const_y),
                                                   round_shift_y);
